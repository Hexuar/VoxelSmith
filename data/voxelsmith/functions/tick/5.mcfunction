...
execute as @e[type=item] if predicate voxelsmith:item/is_ingot if predicate voxelsmith:item/is_not_foreign at @s align xyz positioned ~0.5 ~ ~0.5 unless entity @e[type=item_display,tag=voxelsmith.metal_shape,distance=..0.5,sort=nearest,limit=1] if block ~ ~-1 ~ anvil run function voxelsmith:start_smithing/vanilla
