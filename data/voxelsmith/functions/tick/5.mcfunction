...
execute as @e[type=item,nbt={Item:{components:{"minecraft:custom_data":{voxelsmith:{smithing_ingot:1b}}}}}] at @s align xyz positioned ~0.5 ~ ~0.5 unless entity @e[type=marker,tag=voxelsmith.metal_shape,distance=..0.5,sort=nearest,limit=1] if block ~ ~-1 ~ #anvil run function voxelsmith:start_smithing/main

execute as @e[type=item] if predicate voxelsmith:item/is_knappable if predicate voxelsmith:item/is_not_foreign at @s align xyz positioned ~0.5 ~ ~0.5 unless entity @e[type=marker,tag=voxelsmith.metal_shape,distance=..0.5,sort=nearest,limit=1] if block ~ ~-1 ~ #voxelsmith:knapping_surface run function voxelsmith:start_smithing/vanilla
